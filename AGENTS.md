...
plan 编号是主题编号，不是严格执行队列。**实际顺序只认 `.plan/19-roadmap-and-milestones.md` 的 CR 批次**，当前指针只认 `.plan/README.md` 的「当前执行点」。

当用户只说「开始下一步」「继续开发」时：

1. 读取 `.plan/README.md` 当前执行点。
2. 状态是 `待开发` / `开发中`：只执行当前 CR 批次，不顺手进入下一批次。
3. 状态是 `待 CR`：不要继续写功能，执行当前批次的代码审查。
4. 状态是 `CR 修复中`：只修 review 的阻塞问题，修完回到 `待 CR`。
5. 当前里程碑全部批次完成：先做里程碑集成 Review，不得直接进入下一里程碑。

每次状态变化都同步 `.plan/README.md`；跨阶段 plan（12、14～18）还要更新各自的「子阶段状态」。不要只改聊天里的口头进度。

### 批次边界

- 一个 CR 通常是 1～3 天、一个可独立验证的目标。
- 同一批次允许涉及多个 plan 子阶段，因为测试、安全、CI 本来就必须随功能落地。
- 不允许连续完成多个 CR 后统一 review。
- 测试和安全用例属于实现的一部分，不能留到后续“补测试”批次。
- `04`、`08` 是技术决策门禁，需要按实测更新 ADR；`13-A` 是交付门禁，需要验证真实 dmg。
...
**因此：**任何一次模型调用都必须假设它可能返回垃圾。JSON 输出必须用 zod 校验 + 失败重试 + 兜底路径，三者缺一不可。给模型的工具数量不超过 `capabilities().maxToolCount`（2026-08-26 基线为 6）。多步任务拆成多次单步调用，而不是指望一次输出完整计划。

真实能力边界以 `.plan/04` 的实测报告为准，不要凭印象假设。
