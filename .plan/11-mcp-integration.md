...
- [x] 配置一个不存在的 command，服务正常启动，该 server 标记为连接失败，其他功能不受影响
- [x] 手动重连能恢复连接
- [ ] 停止服务后，`ps` 里没有残留的 MCP server 子进程（需本机对照真实 stdio；代码路径：`onModuleDestroy` + transport `onclose`）
- [x] 工具名冲突时，内置工具优先，MCP 工具带 server 前缀
- [x] 总工具数超过 `maxToolCount` 时被裁剪，且有日志说明裁掉了哪些；执行面拒绝本轮未开放的工具
- [x] MCP 工具返回的内容被当作不可信数据处理（单测隔离声明；未跑真实模型「文件内注入」）
- [x] 记录了挂载 MCP 前后的工具选择正确率对比（见 `.plan/18`：未编造新正确率，引用 04 的 maxToolCount=6）
...
统一对话改造后，天气从 `/chat` 的实时天气意图自动进入工具轮次，不要求开启文件访问。`mcp.json.example` 同时作为预置事实源：启动时只把缺失的 server 补入本地 `mcp.json`，weather 默认关闭，绝不覆盖用户已有 command、路径、开关或过滤器。
