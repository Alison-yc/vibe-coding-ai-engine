...
- [ ] 新建会话、发消息、看到逐字流式输出
- [ ] 刷新页面后历史消息完整恢复
- [ ] 生成中点「停止」，输出立刻停止，已生成内容保留在消息里
- [ ] 生成中直接关闭浏览器标签，服务端日志显示请求被取消（不是继续跑完）
- [ ] 挂载知识库后提问，回答带引用角标，点击能看到原文片段
- [ ] 问知识库里没有的问题，回答「资料中没有相关信息」
- [ ] 让模型输出一段含 `<script>` 或 `<img onerror>` 的内容，页面**不执行**它
- [ ] 让模型生成一个超长回答（1000+ 字），生成过程中页面不卡顿，能正常滚动
- [ ] 生成中的不完整代码块（``` 未闭合）不导致渲染崩溃
- [ ] 关掉 Ollama 后发消息，界面显示可操作的错误提示而不是无限 loading
- [ ] 连续对话 20 轮后仍正常（上下文裁剪生效，没有报 context overflow）
- [ ] 切换主题，对话区所有元素颜色正确（无硬编码色值残留）
