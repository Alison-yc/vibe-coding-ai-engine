...
| -------- | ---------------------------------- |
| 阶段     | M2 · RAG 与对话                    |
| 依赖     | 03、04、05、06、14；07-B 依赖 04-B |
| 预计工期 | 主体 4～5 天；模型切换约 2 天      |
| 状态     | 已完成                             |
...
做出一个体感接近豆包/千问的统一对话页：左侧会话列表，右侧对话区，流式输出，支持挂载知识库问答与引用溯源。日期、计算、UUID、天气无需切换页面；本地文件能力以默认关闭的逐轮开关出现。

**这个页面的 SSE 链路同时承载普通问答、引用、工具状态与审批**，所以要一次做扎实，不要图快。
...
| `tool`      | 工具调用卡片    | 实用工具与文件访问轮次使用  |
| `citation`  | 引用列表        | 点击定位到知识库原文        |

这个设计让普通回答与工具轮次共用同一套渲染管道，不需要各写一套。

### 文件访问开关

`fileAccess` 是每轮请求的最小权限开关，不写入会话；刷新和切换会话后默认关闭。开启时显示工作区与只读/编辑模式，并且本轮不执行知识库检索。关闭时文件工具和 filesystem MCP 都不能进入模型候选或执行面。
...
策略见 ADR-015：白名单无缝切换 + 扫描仅作发现，未知模型只聊天。

实施要点：

1. 契约：创建/更新会话可带 `modelId`；`GET /models` 返回 `{ id, installed, capability, kind: 'evaluated' | 'untested' }`。
2. 网关 `chat` / `stream` / `agentChat` 使用请求中的模型，缺省才用 `OLLAMA_MODEL`。
...
| 对话页中文硬编码                       | 界面文案迁移属于 CR-I18N-B，不要在 07 里另做一套字典。I18N-A 只动导航等壳层                                                                                  |
