...
Dify 的条件节点也是纯代码判断。用 LLM 做条件判断在 2B 模型上不可靠（`04` 的结构化输出测评会证实这点），而且会让工作流变得不可预测——同样的输入两次运行走不同分支，无法调试。
...
- 解析目标域名的 IP，拦截私有网段（127/8、10/8、172.16/12、192.168/16、169.254/16、::1）
- 只允许 http/https 协议
