...
| 模式     | 行为                                              |
| -------- | ------------------------------------------------- |
| 只读模式 | 所有写操作 deny。用于"分析代码"这类场景，安全无忧 |
| 编辑模式 | 写操作 ask。默认模式                              |
...
按 `04` 测出的 `maxToolCount` 裁剪。假设是 3，就按场景动态给：
...
| `grep`  | `pattern`, `path?`               | 按内容搜。**用 `execFile` 传数组参数**调 ripgrep，绝不拼字符串           |
...
- [ ] 让 AI 读取指定文件并总结内容，成功
- [ ] 让 AI 在工作区创建一份 Markdown 文档，文件真的落盘且内容合理
- [ ] 让 AI 修改某文件中的一行，`edit` 工具精确替换成功
- [ ] 让 AI 找出工作区里所有 `.ts` 文件，`glob` 返回正确结果
- [ ] 工具卡片状态正确流转 pending → running → completed

### 安全（每条都要实际试）

- [ ] 让 AI 读 `../../../etc/passwd`，被沙箱拒绝
- [ ] 在工作区放一个指向 `/` 的符号链接，让 AI 读它，被拒绝
- [ ] 让 AI 读 `.env`，触发审批弹窗（不是直接读出来）
- [ ] 让 AI 写文件，审批弹窗显示了将写入的内容
- [ ] 审批点「拒绝」，工具返回错误，模型收到拒绝信息并能继续对话
- [ ] 切到只读模式，让 AI 写文件，被 deny（不是弹审批）
- [ ] `grep` 的 pattern 传 `; rm -rf /`，不发生命令注入（用 execFile 数组参数）

### 健壮性

- [ ] 执行中杀掉服务进程再重启，页面刷新后没有永久 running 的工具卡片
- [ ] 让 AI 做一个需要 5+ 步的任务，到 maxSteps 时强制产出文本回答而不是继续循环
- [ ] 构造一个模型输出 JSON 代码块而非 function call 的情况，兜底解析生效
- [ ] 兜底也失败时，UI 上有明确提示，不是静默答非所问
- [ ] 读一个 10MB 的文件，返回被截断且有明确提示，不是内存爆掉
