...
实现可由统一对话按轮次启用的本地文件 Agent 能力：读文件、写文件、精确编辑、按名找文件、按内容搜索，能生成 Markdown 文档并落盘。带完整的三层安全机制，但不再拥有独立产品页面。
...
| 区域         | 内容                                                                              |
| ------------ | --------------------------------------------------------------------------------- |
| 顶栏         | 默认关闭的文件访问开关；开启后显示工作区目录与只读/编辑模式                       |
...
`fileAccess=false` 时文件工具必须同时从模型候选与执行面移除；不能只靠提示词要求模型“不调用”。同一会话可逐轮切换，历史消息与 tool part 保持连续。
